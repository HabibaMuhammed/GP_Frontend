import React from "react";
import Article from "../../components/article/Article";
import blog02 from "../../assets/blog02.png";
import blog03 from "../../assets/blog03.png";
import blog04 from "../../assets/blog04.png";
import blog05 from "../../assets/blog05.png";
import "./blog.css";

const Blog = () => (
  <div className="gp__blog section__padding" id="blog">
    <div className="gp__blog-heading">
      <h1 className="gradient__text">
        A lot is happening, <br /> We are blogging about it.
      </h1>
    </div>

    <div className="gp__blog-container">
      <br />
      <div className="gp__blog-container_groupB">
        <Article
          imgUrl={blog02}
          date="Sep 25, 2022"
          text=" Security and Open  AI is the future. Let us exlore how it is?"
        />
        <Article
          imgUrl={blog03}
          date="Feb 21, 2023"
          text="Blockchain technology produces a structure of data with inherent security qualities"
        />
        <Article
          imgUrl={blog04}
          date="Sep 26, 2021"
          text="Visual Reverse Engineering of Binary and Data Files"
        />
        <Article
          imgUrl={blog05}
          date="Sep 15, 2022"
          text="A hacker used a compromised Uber EXT account to access the company's internal systems"
        />
      </div>
    </div>
  </div>
);

export default Blog;
